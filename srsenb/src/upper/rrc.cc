...
{
  // nof_sched_infos includes SIB2 by default, plus all configured SIBs
  uint32_t nof_sched_infos = 1+cfg.sibs[0].sib.sib1.N_sched_info;
  LIBLTE_RRC_SCHEDULING_INFO_STRUCT *sched_info = cfg.sibs[0].sib.sib1.sched_info; 
  
  // Allocate DLSCH msg structs, it's array of SIBs
  // SIB1 is in msg[0], and msg[0] only contains SIB1
  // other msg[i] may contain multiple SIBs that all have the same periodicity
  LIBLTE_RRC_BCCH_DLSCH_MSG_STRUCT *msg = (LIBLTE_RRC_BCCH_DLSCH_MSG_STRUCT*)calloc(nof_sched_infos, sizeof(LIBLTE_RRC_BCCH_DLSCH_MSG_STRUCT));
  ...
  // Copy rest of SIBs
  for (uint32_t sched_info_elem = 0; sched_info_elem < nof_sched_infos; sched_info_elem++) {
    ...
    // SIB2 always in first SI message after SIB1
    if (msg_index == 1) {
      msg[1].N_sibs++;
  ...
  for (uint32_t msg_index = 0; msg_index < nof_sched_infos; msg_index++) {
